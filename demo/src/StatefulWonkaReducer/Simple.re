...
let make = (~children) => {
 ...
 let (state, dispatch) = reduce(reducer, state);
  ...
  <div style=containerStyle>
    ...
    <div>
      <button style=leftButtonStyle onClick={_event => {Do(Increment) |> fromValue |> dispatch |> toArray}->ignore}>
        {React.string("+")}
      </button>
      <button style=rightButtonStyle onClick={_event => {Do(Double) |> fromValue |> dispatch |> toArray}->ignore}>
        {React.string("x2")}
      </button>
    </div>
